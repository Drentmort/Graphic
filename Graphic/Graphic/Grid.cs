...
namespace Graphic
{
    ...
    class NumAssign:IDrawer
    {
            ...
            this()
        {
            ...
            if (float.IsNaN(value.X))
            {
                ...
                textLength = value.Y >= 0 ? text.Length*emSize : text.Length * emSize - 1;
            }

            if (float.IsNaN(value.Y))
            {
                text = value.X.ToString(format);
                textLength = value.X >= 0 ? text.Length * emSize : text.Length * emSize - 1;
            }
    ...
    class Grid
    {
        ...
        public void Shift(Point offset)
        {
            ...
            xDownLimit -= xScale * (offset.X / (double)verticalPixelResolution);
        ...
        private void AddHorGridLines()
        {
            ...
            {
                double step = xScale / (double)linesCount;
                double count = xDownLimit / xScale - 1;
                count = (count - (int)count > 0.999) ? (int)count + 1 : (int)count;
                double absXLoc = xScale - xScale * (i / (double)linesCount) + step * (int)count;
        ...
        private void AddVerGridLines()
        {
            for (int i = 0; i < 2 * linesCount + 1; i++)
            {
                double step = yScale / (double)linesCount;
                double count = yUpLimit / yScale - 1;
                count = (count - (int)count > 0.999) ? (int)count + 1 : (int)count;
                double absYLoc = yScale - yScale * (i / (double)linesCount) + step * (int)count;
        ...
        private void AddAssign()
        {
            foreach(var obj in drawer)
            {
                if (obj as GridLine == null)
                    continue;
                assigns.Add(new NumAssign(obj as GridLine, zero, bounds));
            }
