...
namespace Graphic.ChartBox
{
    ...
    class GridLine : IDrawer, IDisposable
    {
        ...
        public virtual void DrawSign(Graphics e, PointF zero, PointF period)
        {}
    ...
    class HorGridLine : GridLine
    {
        ...
        public override void SetVisible(Matrix transform, PointF linePeriod)
        {
            ...
            if (points[0].Y >= -linePeriod.Y &&
                points[0].Y <= field.Height + linePeriod.Y)
                visible = true;
        }
        ...
        public override void DrawSign(Graphics e, PointF zero, PointF period)
        {
            if (!visible)
                return;
    ...
    class VerGridLine : GridLine
    {
        ...
        public override void SetVisible(Matrix transform, PointF linePeriod)
        {
            Point[] points = { start, end };
            transform.TransformPoints(points);
            if (points[0].X >= -linePeriod.X && 
                points[0].X <= field.Width + linePeriod.X)
                visible = true;
        }
